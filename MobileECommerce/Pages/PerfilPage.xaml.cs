...
public partial class PerfilPage : ContentPage
{
    ...
    private async Task<string?> GetImagemPerfil()
    {
        ...
        var (response, errorMessage)
            = await _apiService.GetImagemPerfilUsuario();
            ...
            switch (errorMessage)
            {
                ...
                default:
                    await DisplayAlert("Erro",
                        errorMessage ?? "Nao foi possivel obter a imagem.",
                        "OK");
                    return imagemPadrao;
            }
        }
    ...
    private async void ImgBtnPerfil_OnClicked(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                await DisplayAlert("Erro", "Nao foi possivel carregar a imagem",
                    "Ok");
                return;
            }

            ImgBtnPerfil.Source
                = ImageSource.FromStream(() => new MemoryStream(imagemArray));
            ...
            {
                await DisplayAlert("Erro",
                    response.ErrorMessage ?? "Ocorreu um erro desconhecido",
                    "Cancela");
            }
        }
        catch (Exception ex)
    ...
    private async Task<byte[]?> SelecionarImagemAsync()
    {
        ...
        {
            await DisplayAlert("Erro",
                "A funcionalidade n o   suportada no dispositivo", "Ok");
        }
        catch (PermissionException)
        {
            await DisplayAlert("Erro",
                "Permiss es n o concedidas para acessar a c mera ou galeria",
                "Ok");
        }
        catch (Exception ex)
        {
            await DisplayAlert("Erro",
                $"Erro ao selecionar a imagem: {ex.Message}", "Ok");
        }
